...
%separate dataset without NaN Raads
...
p_theta = (sum(samples(:) == 0))/(mcmc_params.nsamples*3);
...
cov_ASD = [Data.age(1:40)'; Data.edu(1:40)';Data.gender(1:40)']  ;
FIT2.ASDregr = fit_meta_d_mcmc_regression(metaData{1}.nR_S1, metaData{1}.nR_S2, cov_ASD);
cov_CTL = [Data.age(41:end)'; Data.edu(41:end)';Data.gender(41:end)'];  
FIT2.CTLregr = fit_meta_d_mcmc_regression(metaData{2}.nR_S1, metaData{2}.nR_S2, cov_CTL);

%%also accounting for iq (which has 6 NaNs)
...
FIT2.ASDregr = fit_meta_d_mcmc_regression(metaData{1}.nR_S1, metaData{1}.nR_S2, cov_ASD); %%remove ASD: 9, 14, 18, 26, 36
FIT2.CTLregr = fit_meta_d_mcmc_regression(metaData{2}.nR_S1, metaData{2}.nR_S2, cov_CTL); %%remove CTL: 23
cd(scriptDir)
